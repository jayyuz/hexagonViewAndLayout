package com.room.layoutmanagerdemo.ui.scalelayout;
...
public class ScaleLayout extends FrameLayout {
    ...
    /**
     * 所有的down事件都不拦截，因此接下来的move, up事件，都会先执行 onInterceptTouchEvent 的（move, up）
     * 继而分发给子view的dispatchTouchEvent(move, up)，然后在onInterceptTouchEvent（move）事件中判断是否满足滑动条件
     * 满足就拦截，拦截了之后move up事件就会都分发给自身的OnTouchEvent,否则如上继续分发给子View
     *
     * @param ev
     * @return
     */
